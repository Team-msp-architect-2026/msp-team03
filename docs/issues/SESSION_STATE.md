...
기준일: 2026-06-09
수정 이력:
  - 2026-06-10  **팀원 코드 통합(merge) 진행.** 브랜치 `integration/merge1st`(main 무손상) = 워크스트림 A `Aegis-pi`(a9fb5ab) + 내 dashboard `dashboard_vpc`(0a567cd, 6/8) 1차 통합본 위에 내 `0a567cd..main` delta(AI 채팅·이미지 스냅샷 등) 3-way 재적용(코드 충돌 0). 검증 통과: dashboard-backend pytest 223, web lint/test 84/build, 워크스트림 A 앱 115 passed, terraform fmt·data-dashboard/-permanent validate, 런타임 계약 7항목(image_snapshot·processed/{risk_score,state_snapshot}·processed_agg/metrics_5m·DDB latest_image_snapshot·테이블 AEGIS-DynamoDB-FactoryStatus·버킷 aegis-bucket-data·ECS IAM) 생산측↔소비측 일치. tfstate는 S3 `kjw-aegis-terraform-state` 단일 백엔드로 통일(루트별 키 분리). 정리: ADR 번호 충돌 해소(내 arch-overview `0032→0035`, 팀원 `0032-persistent-mqtt` 보존). 본 통합 repo의 `docs/issues/`는 B 추적 문서(M6·MASTER_CHECKLIST·SESSION_STATE)만 보유하며, 워크스트림 A 마일스톤 문서(M0~M5·M7)는 팀원 소스가 source of truth로 본 repo에 중복하지 않는다. 후속: data-pipeline 등 A terraform validate는 terraform 1.15.0+ 환경 필요, 최종 타깃 repo 반영 방식 결정.
...
- `docs/architecture/drawio/agiespi_architecture_overview_final1.drawio` / `images/agiespi_architecture_overview_final3.drawio.png` (overview source of truth, ADR 0035)
...
Tailscale UI: https://100.x.x.x/ for ArgoCD, http://100.x.x.x/ for Grafana
