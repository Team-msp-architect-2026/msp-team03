...
- `factory-b` Tailnet IP: `100.x.x.x`
- `factory-b` worker IP/user: `192.168.x.x`, `oosnim`
- `factory-c` Tailnet IP: `100.x.x.x`
- `factory-c` worker IP/user: `192.168.x.x`, `aegis`
...
**결과**:
`kubectl get nodes -o wide` 실행 시 노드들의 `INTERNAL-IP`가 중복 없이 각각 `192.168.x.x`, `192.168.x.x`으로 올바르게 인식되며, 멀티 노드 간 네트워크 통신이 복구되어 `edge-iot-publisher`의 CoreDNS 질의 및 AWS IoT Core publish가 정상화되었습니다.
