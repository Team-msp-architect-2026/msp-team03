...
Dashboard-relevant processing:
- `factory_state` updates `factory_state`, `risk`, `pipeline_status`, `last_factory_state_at`, and history/state snapshots.
- `infra_state` updates `infra_state`, `pipeline_status`, `last_infra_state_at`, optionally recalculates `risk`, and writes history/state snapshots.
- `image_snapshot` updates `latest_image_snapshot`, `last_image_snapshot_at`, writes processed metadata to S3, and does not send image binary through IoT Core.
