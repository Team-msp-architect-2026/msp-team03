# 05_screen_data_mapping.md 통합 초안
...
기준일: 2026-06-08
수정 이력:
  - 2026-06-08  Timeline/Fleet 10m 원시 조회 limit을 250으로 현행화하고 1h 원시 조회 limit 2000 기준 유지.
...
| UI | API 조회 | 표시 기준 |
| --- | --- | --- |
| `10M` | `GET /factories/{factory_id}/history?window=10m&limit=250` | 기본값. 최근 이상 징후 빠른 확인 |
...
Custom datetime picker는 `min=now-24h`, `max=now`를 적용한다. 따라서 미래 시각과 최신 기준 24시간보다 이전 시각은 선택 UI 단계에서 제외한다.
`datetime-local`은 초 단위를 보존하지 않으므로 1시간 근처 custom 범위에는 60초 grace를 두고 `window=1h` 원시 조회를 사용한다. Empty-state는 데이터가 없는 경우와 데이터는 있으나 risk level/score 변화 조건을 만족하지 않는 경우를 구분한다.
...
```text
GET /factories/{factory_id}/history?window=10m&limit=250
GET /factories/{factory_id}/history?window=1h|<custom-derived-window>&limit=2000
Frontend:
