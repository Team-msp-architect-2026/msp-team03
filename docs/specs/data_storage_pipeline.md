# data_storage_pipeline.md 통합 초안

> Phase 2 source-preserving merge. Same-path Markdown differed between sources.

## [통합 기준 - Phase 4]

DataProcessor/DataPipeline 병합 기준은 아래 계약으로 고정한다.

- DynamoDB hot store 기본 table: `AEGIS-DynamoDB-FactoryStatus`
- `LATEST`: `pk=FACTORY#{factory_id}`, `sk=LATEST`
- History: `pk=FACTORY#{factory_id}`, `sk=HISTORY#STATE#{timestamp}`
- Graph: `GRAPH#5M` read model은 `apps/graph-metrics-aggregator`가 생성한다.
- Pipeline freshness: `PIPELINE#STATUS` 성격의 상태는 `pipeline_status`와 history snapshot으로 Dashboard가 읽을 수 있어야 한다.
- CloudInfra: `pk=CLOUD#infra`, `sk=LATEST`의 `fast`/`slow` 필드를 Dashboard가 읽는다.
- Alerts: `ALERT#...` dedupe/status item은 RiskAlertDispatcher가 관리한다.
- Image snapshot: `latest_image_snapshot`은 binary가 아니라 S3 bucket/key/content metadata를 LATEST item에 연결한다.
- S3 prefixes:
  - `raw/`
  ...
  - `processed_agg/`
  - `reports/daily/`
  - `image_snapshot/factory_id={factory_id}/`
- Retention default for DataProcessor history snapshots: `HISTORY_TTL_HOURS=48`.
- Reporting output contract: `reports/daily/yyyy=YYYY/mm=MM/dd=DD/{factory_id_or_cloud-infra}/report.md`.
- Dashboard Reports API reads the same `report.md` path and treats `cloud-infra` as the cloud infrastructure report target.

Implementation notes:
- `merge/apps/data-processor` is based on the `Aegis-pi` implementation for image snapshot, freshness refresh, infra-aware risk, and factory-b/c support.
- The default DynamoDB table/TTL were aligned with the Dashboard contract from `dashboard_vpc`.
- `merge/apps/cloud-infra-collector` uses the flattened `Aegis-pi` package path required by `infra/data-pipeline`, with Dashboard fields for Redis memory usage and RDS storage preserved.
