...
기준일: 2026-06-17
수정 이력:
  - 2026-06-17  2번 링크 통합 초안의 CloudInfra/RiskAlertDispatcher 저장 계약을 SoT 본문에 흡수. `CLOUD#infra`, `ALERT#`, `processed/cloud_infra/` 기준을 추가.
...
S3 processed/cloud_infra (Cloud infra fast/slow collector 보조 복사본)
...
DynamoDB CLOUD#infra     (Cloud infra latest/history read model)
DynamoDB ALERT#          (alert observation/cooldown/dedupe state)
```
...
EventBridge Scheduler (1분/5분 주기)
  -> Lambda AEGIS-Lambda-CloudInfraFastCollector1m
      -> DynamoDB CLOUD#infra LATEST.fast / HISTORY#FAST
      -> S3 processed/cloud_infra/fast
  -> Lambda AEGIS-Lambda-CloudInfraSlowCollector5m
...
S3 processed ObjectCreated
  -> Lambda AEGIS-Lambda-RiskAlertDispatcher
      -> DynamoDB ALERT#{scope} observation/cooldown/dedupe
...
Dashboard API/Web
  ...
  -> DynamoDB CLOUD#infra     (Cloud infra 현재 상태)
...
| Lambda CloudInfraFastCollector1m | Backend/ECS/ALB/Lambda/DynamoDB/Scheduler/factory freshness 요약 수집 |
| Lambda CloudInfraSlowCollector5m | EKS/Kubernetes/ArgoCD/S3 freshness 요약 수집 |
| Lambda RiskAlertDispatcher | S3 processed snapshot의 warning/danger 조건 평가, 연속 관측 확인, cooldown/dedupe 후 Slack 알림 |
...
| DynamoDB CLOUD#infra | Cloud infra dashboard 현재 상태와 최근 수집 이력 |
| DynamoDB ALERT# | Slack alert 연속 관측 확인 및 중복 알림 억제 상태 |
| S3 raw | Edge data-plane 원본 JSON 장기 보존 |
| S3 processed | Lambda 계산 결과와 상태 요약 이력 보존 |
| S3 processed_agg | GRAPH#5M 보조 JSON 복사본. 장기 재처리용 |
| S3 processed/cloud_infra | Cloud infra fast/slow collector 결과의 장기 보조 복사본 |
...
| `S3 processed/cloud_infra` | Cloud infra fast/slow collector snapshot | 운영 디버깅, 장기 재처리 | 장기 보존 |
...
| `DynamoDB CLOUD#infra` | Cloud infra latest와 fast/slow history | Cloud infra dashboard 현재 상태와 최근 추이 | LATEST overwrite + HISTORY TTL |
| `DynamoDB ALERT#` | RiskAlertDispatcher observation/cooldown/dedupe item | 일시 warning 확인, Slack 중복 억제 | TTL |
...
Cloud infra fast/slow snapshot:

```text
processed/cloud_infra/{fast|slow}/yyyy={YYYY}/mm={MM}/dd={DD}/hh={HH}/{timestamp}.json
```
...
processed/cloud_infra/fast/yyyy=2026/mm=06/dd=04/hh=12/2026-06-04T12:00:00Z.json
```
...
- `processed/cloud_infra/{fast|slow}/`는 DynamoDB `CLOUD#infra` fast/slow snapshot을 장기 보존한다.
...
Cloud infra와 alert item은 같은 테이블에서 아래 key pattern을 사용한다.

| item | pk | sk |
| --- | --- | --- |
| Cloud infra latest | `CLOUD#infra` | `LATEST` |
| Cloud infra fast history | `CLOUD#infra` | `HISTORY#FAST#{timestamp}` |
| Cloud infra slow history | `CLOUD#infra` | `HISTORY#SLOW#{timestamp}` |
| Alert observation/cooldown | `ALERT#{scope}` | 구현별 observation/cooldown/dedupe key |
...
| Cloud infra 현재 상태 | `DynamoDB CLOUD#infra/LATEST` | backend_runtime, data_pipeline, eks_management, storage_freshness |
...
- Lambda CloudInfraFastCollector1m/SlowCollector5m은 `DynamoDB CLOUD#infra`, `S3 processed/cloud_infra`를 담당한다.
- Lambda RiskAlertDispatcher는 `DynamoDB ALERT#` observation/cooldown/dedupe state와 Slack alert routing을 담당한다.
