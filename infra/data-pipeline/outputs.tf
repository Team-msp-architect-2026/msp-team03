...
output "iot_fleet_indexing_enabled" {
  description = "Whether AWS IoT Fleet Indexing is managed and enabled by this root."
  value       = var.iot_fleet_indexing_enabled
}

output "iot_fleet_console_viewer_policy_arn" {
  description = "Read-only IAM policy ARN for AWS IoT Core fleet and connectivity verification."
  value       = var.iot_fleet_console_viewer_policy_enabled ? aws_iam_policy.iot_fleet_console_viewer[0].arn : null
}

output "iot_connectivity_thing_names" {
  description = "Expected Thing names whose MQTT client IDs must match for Fleet Indexing connectivity status."
  value = [
    "${local.naming_prefix}-IoTThing-factory-a",
    "${local.naming_prefix}-IoTThing-factory-b",
    "${local.naming_prefix}-IoTThing-factory-c",
  ]
}
