...
variable "iot_fleet_indexing_enabled" {
  description = "Whether to enable AWS IoT Fleet Indexing for Thing registry and connectivity status."
  type        = bool
  default     = true
}

variable "iot_fleet_console_viewer_policy_enabled" {
  description = "Whether to create a read-only IAM policy for AWS IoT Core fleet/connection verification from console or CLI."
  type        = bool
  default     = true
}

variable "iot_fleet_console_viewer_attach_role_names" {
  description = "Optional IAM role names to attach the IoT fleet console viewer policy to. Leave empty to create the policy without attaching it."
  type        = list(string)
  default     = []
}
