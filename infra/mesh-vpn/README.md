...
관리자 대시보드는 Tailscale에 의존하지 않고 별도 Data / Dashboard VPC에서 제공한다. Dashboard VPC는 DynamoDB LATEST/GRAPH#5M/HISTORY#STATE와 S3 processed/processed_agg를 조회하며 Spoke API를 직접 호출하지 않는다.
...
| --- | --- | --- | --- |
| `factory-a` Raspberry Pi master | `factory-a-master` | `100.x.x.x` | Connected, tagged |
| EKS operator | `tailscale-operator` | `100.x.x.x` | Connected, tagged |
| ArgoCD -> factory-a egress proxy | `argocd-factory-a-master-tailnet` | `100.x.x.x` | Connected, tagged |
| Windows 운영자 PC | `minsoog14` | `100.x.x.x` | Connected |
