...
- `edge-iot-publisher`는 persistent MQTT connection을 유지하며, client ID는 `AEGIS-IoTThing-factory-a`다.
- publisher는 `replicaCount=1`, `strategy=Recreate`를 유지해야 한다.
- snapshot presigner endpoint는 `snapshot-uploader-presign` Secret의 `endpoint` key에서 읽는다. `token` key는 optional이다.

2026-06-09 배포 이미지:

```text
edge-iot-publisher: sha-36eae8c
