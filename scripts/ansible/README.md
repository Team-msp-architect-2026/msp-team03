...
`start_test.yml`은 factory-a master의 Tailscale 상태를 검증 대상에 포함한다. 따라서 SSH 접속 경로로 master Tailscale IP `100.x.x.x`를 쓰면 검증 대상과 실행 경로가 겹친다. Tailscale 장애를 검증하거나 관측해야 할 때는 아래 중 하나를 사용한다.

```text
권장 1: 10.10.10.0/24 내부망에 붙어 있는 control host에서 기본 inventory(10.10.10.10)로 실행
권장 2: master wlan0 관리 IP로 실행
비권장: master Tailscale IP(100.x.x.x)로 실행
```
