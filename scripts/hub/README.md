...
1. MFA OTP 입력
2. AWS MFA session 설정
3. --plan-only: infra/hub terraform init/validate/plan -destroy
4. --yes: Hub platform cleanup
5. --yes: infra/hub saved destroy plan 생성 후 apply
```
...
- `destroy-hub.sh --yes`는 AWS 리소스를 삭제한다.
- 장시간 사용하지 않을 때는 `destroy-hub.sh --plan-only`로 범위를 먼저 확인한 뒤 `destroy-hub.sh --yes`로 제거한다.
