...
`destroy-all.sh`의 기본 destroy 범위는 rebuild 가능한 runtime 계층으로 제한한다.

기본 보존:
- Foundation
- IoT certificate/Thing/Policy
- K3s IoT Secret
- Dashboard permanent resources
- Dashboard DNS

기본 삭제 대상:
- Dashboard runtime: `infra/data-dashboard`
- Reporting
- Data pipeline
- Hub

기본 삭제 대상도 실제 실행 전 state 소유권을 확인해야 한다. 특히 `infra/data-dashboard-permanent`, `infra/data-dashboard-dns`, `infra/foundation`, IoT/K3s Secret은 명시 플래그나 별도 수동 절차 없이 삭제하지 않는다.

Dashboard runtime destroy는 별도 확인이 필요하다.

```bash
scripts/destroy/destroy-data-dashboard.sh --yes [--domain DOMAIN] [--otp MFA_OTP]
```

`destroy-data-dashboard.sh`는 `infra/data-dashboard`만 대상으로 하며 permanent/DNS root는 삭제하지 않는다.
...
`--plan-only`는 `infra/hub/tfplan.destroy-hub`만 생성하고 platform cleanup
또는 Terraform apply를 실행하지 않는다. 실제 삭제는 plan을 검토한 뒤
`--yes`를 명시해야 진행된다.
...
```bash
scripts/destroy/destroy-hub-infra.sh --plan-only [MFA_OTP]
scripts/destroy/destroy-hub-infra.sh --yes [MFA_OTP]
```
...
Persistent MQTT publisher는 Hub를 경유하지 않고 AWS IoT Core에 직접 연결한다. Hub만 삭제해도 Spoke K3s Pod가 살아 있으면 `AEGIS-IoTThing-factory-a/b/c` connectivity는 계속 `connected=true`일 수 있다. Spoke Pod가 재시작되면 정상 종료 시 즉시 disconnected, 강제 종료 시 keepalive timeout 후 disconnected로 바뀌고, 새 Pod가 같은 Thing client ID로 재연결하면 다시 connected가 된다.
...
- `destroy-hub.sh --yes`는 Terraform destroy 전에 반드시 `destroy-hub-platform.sh`(Ingress cleanup)를 먼저 실행한다. Ingress가 비활성화 상태면 cleanup은 no-op에 가깝게 지나간다.
- `destroy-hub.sh --plan-only`는 platform cleanup을 건너뛰고 Hub Terraform destroy plan만 만든다.
- Hub Terraform destroy 대상은 Hub VPC, public/private subnets, route tables, NAT/EIP, Internet Gateway, EKS cluster/node group/addons/KMS resources, IRSA/IAM resources, CloudWatch/EKS related resources다. Foundation, data-pipeline, reporting, Dashboard permanent/DNS, IoT certificate, K3s Secret은 Hub root state 밖이므로 삭제 대상이 아니다.
...
- AWS IoT 연결 상태는 `aws iot get-thing-connectivity-data --thing-name AEGIS-IoTThing-factory-a/b/c`로 확인한다. 전체 기준은 `docs/ops/34_iot_fleet_connectivity.md`를 따른다.
